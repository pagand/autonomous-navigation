...
from tensorflow.compat.v1.keras import backend as backend
...
#sys.path.append('/home/aaa241/Documents/CARLA_0.9.12/PythonAPI/carla/dist/carla-0.9.12-py3.7-linux-x86_64.egg')
...
sys.path.append('/localhome/aaa241/Documents/CARLA_0.9.12/PythonAPI/carla/')
import carla
from agents.navigation.global_route_planner import GlobalRoutePlanner as GlobalRoutePlanner
...
class CarEnv:
    ...
    def reset(self):
        ...
        self.spawn_points = self.client.get_world().get_map().get_spawn_points()
        self.start_location = carla.Location(self.spawn_points[50].location)
        self.end_location = carla.Location(self.spawn_points[100].location)
        self.global_planner = GlobalRoutePlanner(self.map, sampling_resolution = 2)
        self.path = self.global_planner.trace_route(self.start_location, self.end_location)
        i = 0
        for w in self.path:
            if i % 10 == 0:
                self.client.get_world().debug.draw_string(w[0].transform.location, 'O', draw_shadow=False,
        ...
        self.vehicle = self.world.spawn_actor(self.model_3, self.spawn_points[50])
        self.last = 0
        ...
        self.rgb_cam.set_attribute("fov", str(110))  ## fov, field of view
    ...
    def step(self, action):
        ...
        for i in range(2, len(self.path)):
            ...
            if len(self.collision_hist) != 0:
                ...
                reward = -5
            elif carla.Location.distance(carla.Actor.get_location(self.actor_list[0]), (self.path[i])[0].transform.location) == 0:
                done = False
                reward = 25
            else:
                done = False
        ...
        present_location = carla.Actor.get_location(self.actor_list[0])
        

        for i in range(1,len(self.path)):
            dist = carla.Location.distance(carla.Actor.get_location(self.actor_list[0]), (self.path[i])[0].transform.location)
            if dist < 1:
            #if carla.Location.distance(carla.Actor.get_location(self.actor_list[0]), (self.path[i])[0].transform.location) == 0:
                done = False
                reward = r1 + (i-self.last)*r3
                self.last = i
                return self.front_camera, reward, done, None
...
class DQNAgent:
    def __init__(self):
        ...
        self.tensorboard = ModifiedTensorBoard(log_dir="logs/"+MODEL_NAME+"-"+str(int(time.time())))
